...
- **Never makes you wait for Gemini.** The free-tier model is slow and often overloaded, so the endpoint answers straight away with the template tree and `upgrading: true`, then a background job asks Gemini and replaces the cached tree when it succeeds. The dashboard shows the quick tree and swaps in Gemini's automatically. If Gemini fails, the template stays and is retried after 10 minutes.
